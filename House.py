...
df_washrooms = pd.read_csv('AFR.csv')
...
st.header('Affordable Rental House Finder')
...
if st.button("Use Current Location"):
    ...
    if latlng:
        ...
        if closest_washroom is not None:
            st.write(f"Closest Availble Property: {closest_washroom['Property Name']}")
            st.write(f"Address :({closest_washroom['Address']})")
            st.write(f"Zipcode :  {closest_washroom['Zip Code']}")
            st.write(f"Phone Number :  {closest_washroom['Phone Number']}")
        else:
            st.error("No House found near your current location.")
    else:
